...
module.exports = class Ghost {
    ...
    static async getOneGhost(id) {
        const {
            rows,
        } = await pool.query(
            'SELECT * FROM ghosts WHERE id = $1', [id]
        );
        return new Ghost(rows[0]);
    }
    static async changeOneGhost(id, scary) {
        const {
            rows,
        } = await pool.query(
            'UPDATE ghosts SET scary = $1 WHERE id = $2 RETURNING *', [scary, id]
        );
        return new Ghost(rows[0]);
    }
    static async deleteGhost(id) {
        const {
            ...
            'DELETE FROM ghosts WHERE id = $1 RETURNING *', [id]
        );
        return new Ghost(rows[0])
    }
}